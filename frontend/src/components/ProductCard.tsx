...
const ProductCard: FC<ProductCardProps> = ({ 
  ...
  const productData = useMemo(() => {
    ...
    const isBestseller = product.isFeatured || (product.rating && product.rating >= 4.5);
    const isNew = product.isNew;
    const isSale = product.isSale || hasDiscount;
  ...
  const handleColorSelect = useCallback((colorVariant: ProductVariant) => {
  ...
  }, [product, addToCart, onAddToCart, selectedColorVariant]);
  ...
  return (
    <div 
      ...
      <div className="product-image-container">
        ...
        {productData.isNew && product.inStock && (
          <div className="status-badge new">
            New
          </div>
        )}

        {productData.isBestseller && product.inStock && !productData.isNew && (
          <div className="status-badge bestseller">
            Bestseller
          </div>
        )}

        {productData.hasDiscount && product.inStock && (
      ...
      <div className="product-info">
        {/* Brand Name */}
        <div className="brand-name">{productData.brandName}</div>
        ...
        {productData.colorVariants.length > 0 && (
          <div className="colors-available" aria-label="Available colors">
            {productData.colorVariants.map((colorVariant) => (
              <button 
                key={colorVariant.id}
                className={`color-dot ${selectedColorVariant?.id === colorVariant.id ? 'active' : ''}`}
                style={{ backgroundColor: colorVariant.hexColor || colorVariant.value }}
                aria-label={`Select ${colorVariant.name} color`}
                title={colorVariant.name}
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  handleColorSelect(colorVariant);
                }}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
            ...
            {productData.colorVariants.length > 4 && (
              <span className="more-colors">
                +{productData.colorVariants.length - 4} more
              </span>
            )}
          </div>
        )}
        ...
        {/* Available Sizes */}
        {productData.sizeVariants.length > 0 && (
          <div className="sizes-available">
            <span className="sizes-label">Sizes: </span>
            {productData.sizeVariants.slice(0, 5).map((sizeVariant, index) => (
              <span key={sizeVariant.id} className="size-item">
                {sizeVariant.value}
                {index < Math.min(productData.sizeVariants.length - 1, 4) && ', '}
              </span>
            ))}
            {productData.sizeVariants.length > 5 && (
              <span className="more-sizes">
                +{productData.sizeVariants.length - 5} more
              </span>
            )}
          </div>
        )}
