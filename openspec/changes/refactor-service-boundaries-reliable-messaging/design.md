...
Create an `outbox-messaging` Gradle module rather than putting outbox contracts, Spring wiring, codecs, or publishers in `shared-kernel`. The module may depend on `shared-kernel` only for technology-neutral domain-event contracts. Each microservice owns its JPA outbox entity, migration, and `OutboxStore` adapter. The module provides the transaction synchronization, Kafka dispatch orchestration, and Spring Boot auto-configuration. This keeps persistence ownership service-local and prevents the shared kernel from becoming a Spring/Kafka integration module.
